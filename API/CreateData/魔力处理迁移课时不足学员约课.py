...
def get_csv():
    df = pd.read_csv(r'C:\Users\92101\Desktop\课时不足学员.csv', encoding='gbk')
    results = []
...
def get_student_wandouid(class_info, moli_student):
    url = f'{URL}/course/combine/class/cambridge/getClassStudent?class_id={class_info}'
...
def get_eduweb_class_data(class_info, start_time):
    url = f'{URL}/course/combine/class/cambridge/course/list?class_id={class_info}'
...
def main():
    ...
    for (class_info, student_id), start_times in group.items():
        ...
        try:
            resp = requests.post(url=f"{URL}/course/combine/class/cambridge/student/course/add", json=payload, headers=headers)
            if resp.json().get('code') == 200:
                ...
                print(f"部分成功-->班级：{class_info} 学生：{student_id} 课程数：{len(full_course_info)} "
                      f"course_ids={','.join(course_ids)} 原因={resp.json()['message']}")
        except Exception as e:
            print(f"EXCEPT-->班级：{class_info} 学生：{student_id} error={e}")

if __name__ == '__main__':
    init_URL("https://sht-eos-gateway.vipthink.cn")
    init_token("Bearer eyJhbGciOiJzaGEyNTYiLCJ0eXAiOiJKV1QifQ..MjZmM2M2NzQ5OGRiYTA4NWMwMmQyNzg5YjQ5OTk5Zjc1NmFkODE3MDVhODZhNThkOGFiZDlkYTZlNWIyZmRlYg")
    main()
