...
class PushStudentOrder:
    ...
    def input_num(self):
        while True:
            num = input(f"请输入{InputMethods().input_environment}环境需要导入的订单条数:")
            try:
                num = int(num)
            except ValueError:
                print("输入错误，请输入数字")
    ...
    def get_front_sign(self):
        ...
        resp = GeneralMethods().handle_api_response(requests.post(url, json=data, headers={"authorization": self.token}))
    ...
    def import_order(self):
        ...
        if front_sign_data:
            ...
            GeneralMethods().handle_api_response(requests.post(url2, json=data2, headers={"authorization": self.token}))
            data3 = {
    ...
    def yizhi_next_step(self):
        ...
        GeneralMethods().handle_api_response(requests.post(url, json=data, headers={"authorization": self.token}))
    ...
    def get_student_account(self):
        ...
        for i in GetStudentPhone().get_student_phone():
            payload = {"mobile": f"{i}"}
            resp = GeneralMethods().handle_api_response(requests.post(url, json=payload, headers={"authorization": self.token}))
            if resp['code'] == 0:
